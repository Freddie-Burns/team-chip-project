...
def my_log_handler(err_dict):
    if err_dict['type'] == 'err' and err_dict['id'] in fatal_errors:
        raise RuntimeError(
            'Caught LightRig error {:}: {:} at {:} ms'.format(err_dict['id'],
                                                              ERRORS[int(
                                                                  err_dict[
                                                                      'id'])],
                                                              1000 * err_dict[
...
class LightRig(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Defaults
        self.pd_models = [
            None]  # Thorlabs photodiode models supported 'N7747A; PM100D; PM100USB'
        self.pd_serials = [
            None]  # Thorlabs photodiode serials, e.g. 'P2005655'
        ...
        self.log = fifo(
            maxlen=4096)  # Log FIFO of sent commands and received errors
        ...
        self.device_dict = {}
        self.pms = [
            None]  # List of thorlabs photodiode devices can be passed in
        self.laser = None
        ...
        # Setup Qontrol tech
        self.qs = qontrol.MXMotor(serial_port_name=self.m2_serial_port_name,
                                  log_to_stdout=False,
        ...
        if all(element is None for element in self.pms):
            self.pms = []
            for model, serial, unit in zip(self.pd_models, self.pd_serials,
                                           self.units):
                try:
